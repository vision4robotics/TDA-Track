...
class SubDataset(object):
    def __init__(self, name, root, anno, frame_range, num_use, start_idx, is_nat=False):
        ...
        # to better utilize the data, we change the dataset code a little bit
        track_nums = []
        for video in self.videos:
            track_num_per_video = len(meta_data[video])
            track_nums.append(track_num_per_video)
        self.prob = [track_num/sum(track_nums) for track_num in track_nums]
        logger.info(f"{name} dataset has {sum(track_nums)} tracks in total")
    ...
    def shuffle(self):
        ...
        while len(pick) < self.num_use:
            selected_lists = np.random.choice(lists, size=len(lists), p=self.prob)
            pick.extend(selected_lists)
        return pick[:self.num_use]

    def get_image_anno(self, video, track, frame):
        frame = "{:06d}".format(frame)
        zero_track = "{:02d}".format(int(0))
        if self.is_nat:
            image_path = os.path.join(self.root, video, track,
                                      self.path_format.format(frame, zero_track, 'x'))
...
class TrkDataset(Dataset):
    def __init__(self, domain):
        ...
        start = 0
        self.num = 0
        if domain=='source' or domain=='Source':
            for name in cfg.DATASET.SOURCE:
        ...
        elif domain=='target' or domain=='Target':
            for name in cfg.DATASET.TARGET:
                subdata_cfg = getattr(cfg.DATASET, name)
                sub_dataset = SubDataset(
                    name,
                    subdata_cfg.ROOT,
