...
class TCT(nn.Module):
    ...
    def __init__(self,cfg, align_head=False):
        ...
        self.transformer = Transformertime(channel, 6, 1, 2, align_head=self.align_head)
    ...
    def forward(self,x,z,px):
        ...
        # memory_list.append(ppres.unsqueeze(1))
        for i in range(len(px)):
                ...
                if self.align_head:
                    memory = self.transformer.align(memory.permute(1,2,0).reshape(b, c, 11, 11)).view(b,c,-1).permute(2,0,1)
        ...
        ppres,res=self.transformer((res3).view(b,c,-1).permute(2, 0, 1),\
                                     (ppres).view(b,c,-1).permute(2, 0, 1),\
                                     res3.view(b,c,-1).permute(2, 0, 1))
        memory_list.append(ppres.permute(1,2,0).view(b,c,w,h).unsqueeze(1))  # b,t,c,w,h
        ...
        return loc,cls1,cls2,t.cat(memory_list,dim=1)
