...
class Transformertime(Module):
    ...
    def __init__(self, d_model: int = 512, nhead: int = 8, num_encoder_layers: int = 6,
                 num_decoder_layers: int = 6, dim_feedforward: int = 384, dropout: float = 0.1, align_head = False,
        ...
        if self.align_head:
            self.align = Adjust_Transformer(channels=192)
    ...
    def forward(self, src: Tensor, srcc: Tensor,tgt: Tensor, src_mask: Optional[Tensor] = None, tgt_mask: Optional[Tensor] = None,
        ...
        if self.align_head:
            B, C = memory.shape[1], memory.shape[2]
            memory = self.align(memory.permute(1,2,0).reshape(B, C, 11, 11)).view(B,C,-1).permute(2,0,1)
