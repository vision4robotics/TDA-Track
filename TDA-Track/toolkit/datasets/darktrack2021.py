...
class DarkTrack2021_Dataset(Dataset):
    """
    Args:
        name: dataset name
    ...
    def __init__(self, name, dataset_root, load_img=False):
        super(DarkTrack2021_Dataset, self).__init__(name, dataset_root)
        dataset_root = dataset_root
        meta_data = ca(dataset_root)
        dataset_root = dataset_root + '/data_seq'

        long_videos = []
        for video in tqdm(meta_data.keys()):
            # if len(meta_data[video]["gt_rect"])>=1400:
                long_videos.append(video)

        # load videos
        pbar = tqdm(long_videos, desc='loading '+name, ncols=100)
        self.videos = {}
        for video in pbar:
