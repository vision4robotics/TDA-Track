...
parser.add_argument('--cfg', type=str, default='experiments/TDA-Track/config.yaml',
                    help='configuration of tracking')
parser.add_argument('--seed', type=int, default=123456,
                    help='random seed')
parser.add_argument('--local_rank', type=int, default=0,
                    help='compulsory for pytorch launcer')
parser.add_argument('--attention_type', type=str, default="CrossFrameAttention",
                    help='Attention or CrossFrameAttention')
parser.add_argument('--block', type=str, default="AdaptorDisBlock",
                    help='DisBlock or AdaptorDisBlock')
...
def build_data_loader(domain):
    logger.info(f"build {domain} domain train dataset")
    # train_dataset
    train_dataset = TrkDataset(domain)
    logger.info(f"build {domain} domain dataset done")
...
def build_opt_lr(model, current_epoch=0):
    ...
    # if align backbone, set the lr to base_lr_d                      
    try:
        trainable_params += [{'params': filter(lambda x: x.requires_grad,
                                           model.align.parameters()),
                            'lr':  cfg.TRAIN.BASE_LR_d}]
    except:
        logger.info("do not align backbone")

    # if align head, set the lr to base_lr_d  
    for name, param in model.grader.named_parameters():
        if "align" in name:
            trainable_params += [{'params': param, 'lr': cfg.TRAIN.BASE_LR_d}]
            logger.info(f"Learning rate of {name} parameters is set to {cfg.TRAIN.BASE_LR_d}")
        elif "transformer.encoder" in name:
            if current_epoch >= cfg.BACKBONE.TRAIN_EPOCH:
                trainable_params += [{'params': param, 'lr': cfg.TRAIN.BASE_LR_d}]
                logger.info(f"Learning rate of {name} parameters is set to {cfg.TRAIN.BASE_LR_d}")
            else:
                logger.info(f"{name} parameters are frozen")
        else:
            trainable_params += [{'params': param, 'lr': cfg.TRAIN.BASE_LR}]
            logger.info(f"Learning rate of {name} parameters is set to {cfg.TRAIN.BASE_LR}")
...
def train(source_loader, target_loader, model, optimizer, lr_scheduler, tb_writer, Disc, optimizer_D):
    ...
    for idx in range(cfg.TRAIN.EPOCH*num_per_epoch):#    for idx, data in enumerate(train_loader):
        ...
        # train tracker on target domain
        for param in Disc.parameters():
            param.requires_grad = False
        outputs, zf = model(data)
        loss_pse = outputs['total_loss']
        B, T = zf.shape[0], zf.shape[1]
        zf = zf.view(B*T, zf.shape[-3], zf.shape[-2], zf.shape[-1])
        interp = nn.Upsample(size=(128, 128), mode='bilinear', align_corners=True)
        zf_up_t = interp(zf)  # B*T, C, H, W
        zf_up_t = [F.softmax(_zf_up_t, dim=0) for _zf_up_t in zf_up_t]  #[C,H,W]
        zf_up_t = torch.stack(zf_up_t, dim=0)  # B*T, C, H, W 
        zf_up_t = zf_up_t.view(B, T, zf_up_t.shape[-3], zf_up_t.shape[-2], zf_up_t.shape[-1])  # B,T,C,H,W
        if args.attention_type == "Attention":
            D_out_z = torch.stack([Disc(F.softmax(_zf_up_t, dim=1)) for _zf_up_t in zf_up_t]).sum(0) / 5.
        else:
            D_out_z = Disc(zf_up_t).sum(0) / 5.
        D_source_label = torch.FloatTensor(D_out_z.data.size()).fill_(source_label)
        loss_adv = 0.1 * weightedMSE(D_out_z, D_source_label)  #  / cfg.TRAIN.BATCH_SIZE
        if is_valid_number(loss_adv.data.item()):
            optimizer.zero_grad()
            optimizer_D.zero_grad()
            loss_adv.backward()

        # train tracker on source domain
        data = source_data.__next__()[1]
        outputs, zf = model(data)
        B, T = zf.shape[0], zf.shape[1]
        zf = zf.view(B*T, zf.shape[-3], zf.shape[-2], zf.shape[-1])
        interp = nn.Upsample(size=(128, 128), mode='bilinear', align_corners=True)
        zf_up_s = interp(zf)  # B*T, C, H, W
        zf_up_s = [F.softmax(_zf_up_s, dim=0) for _zf_up_s in zf_up_s]  #[C,H,W]
        zf_up_s = torch.stack(zf_up_s, dim=0)  # B*T, C, H, W 
        zf_up_s = zf_up_s.view(B, T, zf_up_s.shape[-3], zf_up_s.shape[-2], zf_up_s.shape[-1])  # B,T,C,H,W
        ...
        if args.attention_type == "Attention":
            D_out_1 = torch.stack([Disc(F.softmax(_zf_up_t, dim=1)) for _zf_up_t in zf_up_t]).sum(0) / 5.
        else:
            D_out_1 = Disc(zf_up_t).sum(0) / 5.
        D_target_label = torch.FloatTensor(D_out_1.data.size()).fill_(target_label)
        loss_d = 0.1*weightedMSE(D_out_1, D_target_label) 
        ...
        if args.attention_type == "Attention":
            D_out_1 = torch.stack([Disc(F.softmax(_zf_up_s, dim=1)) for _zf_up_s in zf_up_s]).sum(0) / 5.
        else:
            D_out_1 = Disc(zf_up_s).sum(0) / 5.
        D_source_label = torch.FloatTensor(D_out_1.data.size()).fill_(source_label)
        loss_d = 0.1*weightedMSE(D_out_1, D_source_label)
        if is_valid_number(loss_d.data.item()):
            loss_d.backward() 
        loss_train_adv += loss_d.item()
...
def main():
    ...
    # create model
    model = ModelBuilder_tctrack(label='train', align_backbone=False, align_head=False).cuda().train()
    ...
    model_Disc = TransformerDiscriminator(channels=192, attention_type=args.attention_type, block=args.block)
    model_Disc.train()
    model_Disc.cuda()
